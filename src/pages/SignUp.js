...
const SignUp = () => {
  ...
  const checkPasswordStrength = (password) => {
    /* ... [same function as before] ... */ const checks = {
  ...
  const handleChange = (e) => {
    /* ... [same function as before] ... */ const { name, value } = e.target;
  ...
  const handleSubmit = async (e) => {
    /* ... [same function as before, including consent checks] ... */ e.preventDefault();
    setError("");
    if (!agreedToTerms) {
  ...
  const formItemVariants = {
    hidden: { opacity: 0, y: 20 }, // Start transparent and slightly lower
    visible: {
      opacity: 1,
      y: 0,
      transition: { type: "spring", stiffness: 100, damping: 12 }, // Springy effect
    },
  };
  ...
  return (
    <div style={{ display: "flex", height: "100vh", width: "100vw", overflow: "hidden" }}>
      <div className="left">
        {/* Apply variants to the form container */}
        <motion.div
        ...
        >
          ...
          {/* Form remains largely the same, but wrap labels/inputs/buttons in motion.div */}
            ...
            <AnimatePresence>
              {formData.password && (
                ...
                >
                  ...
                  {/* Wrap Requirement grid in motion.div for stagger */}
                  <motion.div
                    variants={formContainerVariants}
                    initial="hidden"
                    animate="visible"
