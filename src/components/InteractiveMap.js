...
const InteractiveMap = ({
  ...
  useEffect(() => {
    ...
    const setupMap = async () => {
      try {
        ...
        // --- BOSS FIX: Manually reset Leaflet's default icon paths ---
        // This fixes issues where Webpack breaks the paths and markers disappear.
        ...
        const createIcon = (colorUrl) => {
          try {
            ...
            // This fallback will now work because of the fix above
        ...
        if (userLocation && userLocation.lat != null && userLocation.lng != null) {
          try {
            ...
            const routeData = await calculateDistance(userLocation, resolvedCoordinates, "drive");
            ...
            if (
              ...
              const routeBounds = L.latLngBounds(routeData.polylineCoordinates);
              const userLatLng = [userLocation.lat, userLocation.lng];
              const destLatLng = [resolvedCoordinates.lat, resolvedCoordinates.lng];
              ...
              if (addedMarkerCoords.length > 0) {
                const markerBounds = L.latLngBounds(addedMarkerCoords);
  ...
  }, [resolvedCoordinates, name, address, userLocation, nearbyPlaces, iconUrls, onRouteCalculated]);
